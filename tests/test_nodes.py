...
from tests.fakes import partial_state, thread_config


def test_check_required_writes_missing_fields():
    out = check_required(partial_state(request=TravelRequest(), missing_fields=[]))
...
async def test_ask_missing_interrupts_then_resumes():
    ...
    async with AsyncSqliteSaver.from_conn_string(":memory:") as saver:
        ...
        r1 = await graph.ainvoke(
            partial_state(request=TravelRequest(), messages=[], missing_fields=[]), config
        )
